...
public class AlarmRuleQuery {
    ...
    private static List<Rule> transform(String[] auth) {
        ...
        List<Rule> ruleList = Collections.synchronizedList(new ArrayList<>());
        String repsBody = HttpRequest.post(DOMAIN + URL.QUERY_RULE).header(Header.AUTHORIZATION, auth[0])
                .header("SESSION_ID", "SESSION_ID=" + auth[1]).header(Header.CONTENT_TYPE, ContentType.JSON.getValue())
        ...
        for (Object obj : jsonArray) {
            ...
            ruleList.add(Rule.builder().fid(fid).fname(fname).fcategory(fcategory).fkillchain(fkillchain).conditions(conditions)
                                 .fsubcategory(fsubcategory).build());
        }
        return ruleList.stream().peek(e -> e.setSortId(Integer.valueOf(e.getFid()))).sorted(Comparator.comparing(Rule::getSortId))
    ...
    private static void writeExcel(String geneFileName, List<Rule> ruleList) {
        ...
        FileUtil.del(file);
        ExcelWriter writer = ExcelUtil.getWriter(file).addHeaderAlias("fid", "策略ID").addHeaderAlias("fname", "策略名称")
                .addHeaderAlias("fcategory", "告警分类").addHeaderAlias("fsubcategory", "告警子类别").addHeaderAlias("conditions", "条件预览")
    ...
    private static String[] login(String auth) {
        ...
        HttpResponse reps = HttpRequest.post(DOMAIN + URL.LOGIN).header(Header.CONTENT_TYPE, ContentType.JSON.getValue())
                .body(JSONUtil.createObj().set("type", "account").set("pass", password).set("user", userName).toString()).execute();
